...
class Trainer(object):
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
            RS_valid_logging_output = {}
            for lang_pair in cfg.model.reaction_pairs.split(','):
                RS_valid_logging_output[lang_pair + '_loss'] = logging_outputs[0][
                    lang_pair + '_loss']
                RS_valid_logging_output[lang_pair + '_size'] = logging_outputs[0][
                    lang_pair + '_size']
        ...
        if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
            for lang_pair in cfg.model.reaction_pairs.split(','):
                logging_output[lang_pair + '_loss'] = RS_valid_logging_output[
                    lang_pair + '_loss']
                logging_output[lang_pair + '_size'] = RS_valid_logging_output[
                    lang_pair + '_size']
    ...
    def _init_ema_model(self):
        ...
        if self._ema_device == "cpu":
            self._ema_model = self._ema_model.cpu()
        else:
            # Get the device of the main model
            main_model_device = next(self._model.parameters()).device
            self._ema_model = self._ema_model.to(main_model_device)
        ...
        # Log EMA initialization details
        ema_device = next(self._ema_model.parameters()).device
        logger.info(f"EMA model initialized on device: {ema_device}")
        logger.info(f"EMA decay: {self._ema_decay}, FP32: {self._ema_fp32}")

    def update_ema(self):
        """Update EMA model parameters."""
        if self._ema_model is None or self._ema_decay <= 0:
            return

        # Get current model parameters
        model_params = dict(self._model.named_parameters())
        model_buffers = dict(self._model.named_buffers())

        # Update EMA parameters
        with torch.no_grad():
            for name, param in self._ema_model.named_parameters():
                if name in model_params:
                    # Move model param to EMA device if needed
                    model_param = model_params[name]
                    if model_param.device != param.device:
                        model_param = model_param.to(param.device)

                    # Apply EMA update
                    if self._ema_fp32:
                        param.data = param.data.float()
                        param.data.mul_(self._ema_decay).add_(
                            model_param.data.float(), alpha=1.0 - self._ema_decay
                        )
                    else:
                        param.data.mul_(self._ema_decay).add_(
                            model_param.data, alpha=1.0 - self._ema_decay
                        )

            # Update EMA buffers (e.g., batch norm statistics)
            for name, buffer in self._ema_model.named_buffers():
                if name in model_buffers:
                    model_buffer = model_buffers[name]
                    if model_buffer.device != buffer.device:
                        model_buffer = model_buffer.to(buffer.device)
                    buffer.data.copy_(model_buffer.data)
