...
class LabelSmoothedCrossEntropyCriterionWithTraceNormRegularization(
    ...
    def __init__(self, task, sentence_avg, label_smoothing,
        ...
        if task.args.debug_mode:
            pdb.set_trace()
        self.source_reactions = task.source_reactions
        if len(task.source_reactions) > 1:
            source_reactionuage_specific_words_indices = {}
            for lang in task.source_reactions:
                # 1. loading language-specific dicts
                RS_dict = TranslationMultiSimpleEpochTask.load_dictionary(f"{task.args.data}/RS_dict.{lang}.txt")
                RS_dict.add_symbol(f"__{lang}__")

                # 2. constructing a mask vector to zero the probs in other reactions.
                indices = []
                src_dict = task.dicts[lang]
                for i in range(len(src_dict)):
                    if src_dict[i] in RS_dict:
                        indices.append(i)
                indices = torch.tensor(indices, requires_grad=False)
                source_reactionuage_specific_words_indices[lang] = indices
            self.source_reactionuage_specific_words_indices = source_reactionuage_specific_words_indices
        if task.args.debug_mode:
            pdb.set_trace()

        self.target_reactions = task.target_reactions
        if len(task.target_reactions) > 1:
            target_reactionuage_specific_words_indices = {}
            for lang in task.target_reactions:
                # 1. loading language-specific dicts
                RS_dict = TranslationMultiSimpleEpochTask.load_dictionary(f"{task.args.data}/RS_dict.{lang}.txt")
                RS_dict.add_symbol(f"__{lang}__")
                ...
                for i in range(len(tgt_dict)):
                    if tgt_dict[i] in RS_dict:
                        indices.append(i)
                indices = torch.tensor(indices, requires_grad=False)
                target_reactionuage_specific_words_indices[lang] = indices
            self.target_reactionuage_specific_words_indices = target_reactionuage_specific_words_indices
        if task.args.debug_mode:
            pdb.set_trace()
    ...
    def calculate_trace_norm_loss(self, student):
        ...
        trace_norm_1 = 0
        if len(self.source_reactions) > 1:
            trace_norm_1 += calculate_trace_norm(source_embed) * self.w1

        # 2. source Intra-Language Trace-Norm
        trace_norm_2 = 0
        for lang in self.source_reactions:
            LS_source_embed = source_embed[self.source_reactionuage_specific_words_indices[lang]]
            trace_norm_2 += calculate_trace_norm(LS_source_embed) * self.w2

        # 3. target Inter-Language Trace Norm
        trace_norm_3 = 0
        if len(self.target_reactions) > 1:
            trace_norm_3 += calculate_trace_norm(target_embed) * self.w3

        # 4. target Intra-Language Trace Norm
        trace_norm_4 = 0
        for lang in self.target_reactions:
            LS_target_embed = target_embed[self.target_reactionuage_specific_words_indices[lang]]
