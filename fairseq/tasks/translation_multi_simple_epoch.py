...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        # fmt: off
        parser.add_argument('-s', '--source-reaction', default=None, metavar='SRC',
                            help='inference source language')
        parser.add_argument('-t', '--target-reaction', default=None, metavar='TARGET',
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        if training:
            ...
            self.reaction_pairs = ["{}-{}".format(args.source_reaction, args.target_reaction)]
        # eval_lang_pairs for multilingual translation is usually all of the
        # lang_pairs. However for other multitask settings or when we want to
        # optimize for certain languages we want to use a different subset. Thus
        # the eval_lang_pairs class variable is provided for classes that extend
        # this class.
        self.eval_lang_pairs = self.reaction_pairs
        # model_lang_pairs will be used to build encoder-decoder model pairs in
        # models.build_model(). This allows multitask type of sub-class can
        # build models other than the input lang_pairs
        self.model_lang_pairs = self.reaction_pairs
        self.source_reactions = [d.split("-")[0] for d in self.reaction_pairs]
        self.target_reactions = [d.split("-")[1] for d in self.reaction_pairs]
        self.check_dicts(self.dicts, self.source_reactions, self.target_reactions)
        ...
        self.RS_distillation_weights = {}

    def check_dicts(self, dicts, source_reactions, target_reactions):
        if self.args.source_dict is not None or self.args.target_dict is not None:
            ...
            return
        src_dict = dicts[source_reactions[0]]
        tgt_dict = dicts[target_reactions[0]]
        for src_lang in source_reactions:
            assert (
        ...
        for tgt_lang in target_reactions:
            assert (
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        ...
        if self.args.lang_tok_replacing_bos_eos:
            dataset = self.data_manager.alter_dataset_langtok(
                ...
                src_lang=self.args.source_reaction,
                tgt_eos=self.target_dictionary.eos(),
                tgt_lang=self.args.target_reaction,
                src_langtok_spec=src_langtok_spec,
                tgt_langtok_spec=tgt_langtok_spec,
            )
        else:
            dataset.src = self.data_manager.src_dataset_tranform_func(
                self.args.source_reaction,
                self.args.target_reaction,
                dataset=dataset.src,
        ...
        if not getattr(args, "keep_inference_langtok", False):
            ...
            if tgt_langtok_spec:
                tgt_lang_tok = self.data_manager.get_decoder_langtok(
                    self.args.target_reaction, tgt_langtok_spec
                )
    ...
    def source_dictionary(self):
        return self.data_manager.get_source_dictionary(self.source_reactions[0])

    @property
    def target_dictionary(self):
        return self.data_manager.get_target_dictionary(self.target_reactions[0])
    ...
    def train_step(
        ...
        alpha = self.RS_distillation_weights[batch_lang_pair] \
    ...
    def init_RS_distillation_weights(self):
        lang_pairs = self.reaction_pairs
        if self.args.mutual_distillation_mode == "automatic":
            for lang_pair in lang_pairs:
                self.RS_distillation_weights[lang_pair] = 0.1
        elif self.args.mutual_distillation_mode == "bidirectional":
            if self.args.pre_RS_distillation_weight is not None:
                pre_RS_distillation_weight = json.loads(self.args.pre_RS_distillation_weight.replace("'", '"'))
                for lang_pair in lang_pairs:
                    self.RS_distillation_weights[lang_pair] = pre_RS_distillation_weight[lang_pair]
            else:
                for lang_pair in lang_pairs:
                    self.RS_distillation_weights[lang_pair] = self.args.distillation_weight
        elif self.args.mutual_distillation_mode == "unidirectional":
            for lang_pair in lang_pairs:
                self.RS_distillation_weights[lang_pair] = 0
            ...
            #     high_resource_reactions = ["fr", "de", "zh"]
            #     low_resource_reactions = ["tr", "et", "ro"]
            # else:
            #     raise Exception(f"Unknown dataset name {dataset}")
            # high_resource_pairs = [i + '-en' for i in high_resource_reactions] + ['en-' + i for i in high_resource_reactions]
            # low_resource_pairs = [i + '-en' for i in low_resource_reactions] + ['en-' + i for i in low_resource_reactions]
            #
            ...
            #             self.RS_distillation_weight[lang_pair] = self.args.distillation_weight
