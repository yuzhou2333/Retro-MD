...
class OnlineBackTranslationTask(TranslationTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--valid-reaction-pairs', default=None, metavar='VALID_LANG_PAIRS',
    ...
    def get_bos_token_from_sample(self, sample):
        ...
        source_reaction_token_id = torch.unique(net_input["src_tokens"][:, 0]).item()
        source_reaction_token = self.dictionary[source_reaction_token_id].replace("_", "")
        target_reaction_token_id = _lang_token_index(
            self.dictionary, self.get_other_lang(source_reaction_token)
        )

        return target_reaction_token_id
