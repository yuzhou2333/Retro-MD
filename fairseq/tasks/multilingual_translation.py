...
class MultilingualTranslationTask(LegacyFairseqTask):
    ...
    During inference it is required to specify a single `--source-reaction` and
    `--target-reaction`, which indicates the inference langauge direction.
    ...
    def prepare(cls, args, **kargs):
        ...
        if args.source_reaction is not None or args.target_reaction is not None:
            training = False
        else:
            training = True
    ...
    def load_dataset(self, split, epoch=1, **kwargs):
        ...
        self.datasets[split] = RoundRobinZipDatasets(
            ...
            eval_key=None
            if self.training
            else "%s-%s" % (self.args.source_reaction, self.args.target_reaction),
        )
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, constraints=None):
        ...
        lang_pair = "%s-%s" % (self.args.source_reaction, self.args.target_reaction)
        ...
        with torch.no_grad():
            if self.args.decoder_langtok:
                bos_token = _lang_token_index(
                    self.target_dictionary, self.args.target_reaction
                )
            else:
    ...
    def source_dictionary(self):
        if self.training:
            ...
            return self.dicts[self.args.source_reaction]
    ...
    def target_dictionary(self):
        if self.training:
            ...
            return self.dicts[self.args.target_reaction]
    ...
    def max_positions(self):
        ...
        if len(self.datasets.values()) == 0:
            return {
                "%s-%s"
                % (self.args.source_reaction, self.args.target_reaction): (
