...
class SelfDistillationManager(object):
    ...
    def __init__(self, cfg, trainer, task, epoch_iter):
        ...
        self.reaction_pairs = cfg.model.reaction_pairs.split(',')
    ...
    def update_teacher_models(self, RS_valid_loss, valid_loss, current_epoch):
        self.current_epoch = current_epoch

        logger.info(f"current epoch: {self.current_epoch}")
        # 1. update RS_best_models
        for lang_pair in self.reaction_pairs:
            if self.RS_best_models[lang_pair]["valid_loss"] > RS_valid_loss[lang_pair]:
                self.RS_best_models[lang_pair]["valid_loss"] = RS_valid_loss[lang_pair]
                self.RS_best_models[lang_pair]["epoch"] = self.current_epoch

    def read_RS_best_models(self):
        # Read loss curve (history) of each language.
        RS_valid_loss_history = None
        if self.current_epoch > 1:
            f = open(self.model_path + '/RS_valid_loss_history.json')
        ...
        if RS_valid_loss_history is not None:
            for lang_pair in self.reaction_pairs:
                RS_best_loss = min(RS_valid_loss_history[lang_pair])
                RS_best_epoch = 1 + RS_valid_loss_history[lang_pair].index(RS_best_loss)

                RS_best_models[lang_pair] = {
                    "epoch": RS_best_epoch,
...
class LSSD_Manager(SelfDistillationManager):
    ...
    def __init__(self, cfg, trainer, task, epoch_iter):
        ...
        if self.MAX_Teacher_NUM == -1:
            self.MAX_Teacher_NUM = len(self.reaction_pairs)
    ...
    def _init_teacher_models(self):
        ...
        """
        # 1. init RS_best_models
        RS_best_models = self.read_RS_best_models()

        # 2. clustering teacher models
        all_RS_best_epochs = [RS_best_models[lang_pair]["epoch"] for lang_pair in self.reaction_pairs]
        teacher_clusters = self._cluster_teacher_models(all_RS_best_epochs)
        if self.trainer.is_data_parallel_master:
            print("Teacher model clustering")
            print(f"RS_best_epochs:   | {all_RS_best_epochs}")
        ...
        logger.debug(self.reaction_pairs)
        RS_teacher_models = {}.fromkeys(self.reaction_pairs)
        logger.debug(RS_teacher_models)
        for lang_pair in self.reaction_pairs:
            RS_teacher_models[lang_pair] = None
            for cluster_id, cluster in enumerate(teacher_clusters):
                if RS_best_models[lang_pair]['epoch'] in cluster:  # Find the corresponding cluster
                    RS_teacher_models[lang_pair] = teacher_model_pool[cluster_id]
                    break

        self.RS_best_models = RS_best_models
        self.teacher_clusters = teacher_clusters
        self.teacher_model_pool = teacher_model_pool
        self.RS_teacher_models = RS_teacher_models

    def update_teacher_models(self, RS_valid_loss, valid_loss, current_epoch):
        super().update_teacher_models(RS_valid_loss, valid_loss, current_epoch)
        self._update_teacher_models(RS_valid_loss)

    def _update_teacher_models(self, RS_valid_loss):
        if self.fixed_teacher:
            return

        # 2. update teacher_clusters, e.g., re-clustering RS teacher epochs
        all_RS_best_epochs = [self.RS_best_models[lang_pair]["epoch"] for lang_pair in self.reaction_pairs]
        new_teacher_clusters = self._cluster_teacher_models(all_RS_best_epochs)
        if self.trainer.is_data_parallel_master:
            print("Teacher model clustering")
            print(f"RS_best_epochs:       | {all_RS_best_epochs}")
        ...
        # 4. update RS_teacher_models
        RS_teacher_models = {}
        for lang_pair in self.reaction_pairs:
            RS_teacher_models[lang_pair] = None
            for cluster_id, cluster in enumerate(new_teacher_clusters):
                if self.RS_best_models[lang_pair]['epoch'] in cluster:  # Find the corresponding cluster
                    RS_teacher_models[lang_pair] = new_teacher_model_pool[cluster_id]
                    break
    ...
    def _cluster_teacher_models(self, RS_teacher_epochs):
        """
            The implementation of teacher epoch clustering
        """
        # filter valid teacher epochs
        RS_teacher_epochs = [i for i in RS_teacher_epochs if 0 < i]
        if not self.fixed_teacher and len(RS_teacher_epochs) > 0:
            assert max(RS_teacher_epochs) <= self.current_epoch, \
                "Language-Specific best epoch should not be greater than the current epoch."
            RS_teacher_epochs = [i for i in RS_teacher_epochs if i < self.current_epoch]
        RS_teacher_epochs = sorted(RS_teacher_epochs)

        if len(set(RS_teacher_epochs)) <= self.MAX_Teacher_NUM:
            # each unique LS teacher epoch is a cluster
            return [[i] for i in sorted(set(RS_teacher_epochs))]

        intervals = [RS_teacher_epochs[i+1] - RS_teacher_epochs[i]
                     for i in range(len(RS_teacher_epochs) - 1)]
        ...
        for i in range(len(selected_interval_indices) - 1):
            ...
            teacher_clusters.append(RS_teacher_epochs[begin_index: end_index])
    ...
    def get_teacher_model(self, lang_pair=None):
        ...
        return self.RS_teacher_models[lang_pair]
    ...
    def get_distillation_switch_status(self, lang_pair=None):
        ...
        RS_best_epoch = self.RS_best_models[lang_pair]["epoch"]
        if cur_epoch > self.self_distillation_warmup_epoch \
                and cur_epoch - RS_best_epoch > self.inspection_epoch_NUM:
            return True
        else:
            return False
...
class STSD_Manager(SelfDistillationManager):
    ...
    def _init_teacher_model(self):
        ...
        RS_valid_loss_history = None
        if self.current_epoch > 1:
            f = open(self.checkpoint_path + '/RS_valid_loss_history.json')
            RS_valid_loss_history = json.load(f)
            f.close()
        ...
        if RS_valid_loss_history is not None:
            overall_best_loss = min(RS_valid_loss_history["all"])
            overall_best_epoch = 1 + RS_valid_loss_history["all"].index(overall_best_loss)
            overall_best_model = {
...
class KD_Manager(SelfDistillationManager):
    ...
    def __init__(self, cfg, trainer, task, epoch_iter):
        ...
        self.RS_last_models = None

        # initialize the above key data structs
        self._init_RS_last_models()

    def _init_RS_last_models(self):
        """
        Initialize language-specific teacher models.
        """

        # 1. init RS_last_models
        RS_valid_loss_history = None
        if self.current_epoch > 1:
            f = open(self.model_path + '/RS_valid_loss_history.json')
            RS_valid_loss_history = json.load(f)
        ...
        if RS_valid_loss_history is not None:
            for lang_pair in self.reaction_pairs:
                RS_last_loss = RS_valid_loss_history[lang_pair][-1]

                RS_last_models[lang_pair] = {
                    "valid_loss": RS_last_loss
                }
        else:
            for lang_pair in self.reaction_pairs:
                RS_last_models[lang_pair] = {
        ...
        fixed_teacher_loss = {}
        for lang_pair in self.reaction_pairs:
            RS_teacher_models[lang_pair], fixed_teacher_loss[lang_pair] = self._load_model(
                f"{self.fixed_teacher_model_path}/bilingual_{lang_pair}/checkpoint_best.pt"
            )

        self.RS_last_models = RS_last_models
        self.RS_teacher_models = RS_teacher_models
    ...
    def update_teacher_models(self, RS_valid_loss, valid_loss, current_epoch):
        if self.cfg.task.debug_mode:
            pdb.set_trace()
        for lang_pair in self.reaction_pairs:
            self.RS_last_models[lang_pair]["valid_loss"] = RS_valid_loss[lang_pair]
    ...
    def get_distillation_switch_status(self, lang_pair=None):
        ...
        if self.cfg.task.debug_mode:
            pdb.set_trace()
        assert lang_pair is not None, "the language-pair argument shouldn't be None"
        return self.fixed_teacher_loss[lang_pair] <= self.RS_last_models[lang_pair]["valid_loss"]