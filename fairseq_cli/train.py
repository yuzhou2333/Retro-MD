...
def main(cfg: FairseqConfig) -> None:
    ...
    if distributed_utils.is_master(cfg.distributed_training) and cfg.common.use_wandb and cfg.common.wandb_project:
        try:
            ...
            logger.info(f"Attempting to initialize wandb in {wandb_mode} mode...")
            
            # 初始化wandb，添加更严格的超时和错误处理
            import signal
            
            def timeout_handler(signum, frame):
                raise TimeoutError("Wandb initialization timeout")
            ...
            try:
                wandb.init(
                    ...
                    settings=wandb.Settings(
                        ...
                        _disable_meta=True,
                        console="off",
                        git_remote=None,  # 禁用git跟踪
                        disable_git=True,  # 完全禁用git
                        _disable_service=True,  # 禁用service进程
                    )
                )
                signal.alarm(0)  # 取消超时
                logger.info(f"✅ Wandb initialized successfully: {wandb.run.name} for project: {cfg.common.wandb_project}")
                
            except TimeoutError:
                signal.alarm(0)
                logger.warning("⏰ Wandb initialization timeout, trying offline mode...")
            ...
            # 设置一个标志，表示wandb未成功初始化
            cfg.common.wandb_project = None
    ...
    cfg.task.pre_RS_distillation_weight = cfg.task.pre_RS_distillation_weight_model1
    cfg2.task.pre_RS_distillation_weight = cfg.task.pre_RS_distillation_weight_model2
    ...
    task.init_RS_distillation_weights()
    task2.init_RS_distillation_weights()
...
def train(
    ...
    logger.info(f"model-1 distillation weights: {task.RS_distillation_weights}")
    logger.info(f"model-2 distillation weights: {task2.RS_distillation_weights}")
    ...
    # the itr is iterators.CountingIterator
    update_freq = (
        cfg.optimization.update_freq[epoch_itr.epoch - 1]
        if epoch_itr.epoch <= len(cfg.optimization.update_freq)
        else cfg.optimization.update_freq[-1]
    )
    # The grouped iterator will return a list of update_freq batches
    # the list includes update_freq batches of data
    ...
    for i, samples in enumerate(progress):
        ...
        if end_of_epoch:
            save_RS_distillation_weights(cfg, trainer)
            save_RS_distillation_weights(cfg2, trainer2)
...
def validate(
    ...
    if getattr(cfg.task, "RS_epoch", None):
        ...
        for lang_pair in cfg.model.reaction_pairs.split(','):
            RS_valid_cumulative_loss[lang_pair] = {
                'cumulative_loss': 0.0,
                'cumulative_size': 0
            }
        ...
        with metrics.aggregate(new_root=True) as agg:
            for i, sample in enumerate(progress):
                ...
                if getattr(cfg.task, "RS_epoch", None) and (sample is not None and len(sample) > 0):
                    ...
                    for lang_pair in cfg.model.reaction_pairs.split(','):
                        RS_valid_cumulative_loss[lang_pair]["cumulative_loss"] += round(
                            logging_output[lang_pair + '_loss'].item() / math.log(2), 3)
                        RS_valid_cumulative_loss[lang_pair]["cumulative_size"] += logging_output[
                            lang_pair + '_size']
                else:
...
def update_distillation_weight(cfg1, trainer1, task1, epoch_itr1,
    ...
    if cfg1.task.mutual_distillation_mode == "unidirectional" \
        ...
        f = open(f"{cfg1.checkpoint.save_dir}/RS_valid_loss_history.json")
        RS_valid_loss_history1 = json.load(f)
        f.close()
        RS_valid_loss_history1.pop("all")

        f = open(f"{cfg2.checkpoint.save_dir}/RS_valid_loss_history.json")
        RS_valid_loss_history2 = json.load(f)
        f.close()
        RS_valid_loss_history2.pop("all")

        tolerance = 0.05
        for lang_pair in RS_valid_loss_history1.keys():
            if RS_valid_loss_history1[lang_pair][-1] + tolerance < RS_valid_loss_history2[lang_pair][-1]:
                task1.RS_distillation_weights[lang_pair] = 0
            else:
                task1.RS_distillation_weights[lang_pair] = cfg1.task.distillation_weight

            if RS_valid_loss_history2[lang_pair][-1] + tolerance < RS_valid_loss_history1[lang_pair][-1]:
                task2.RS_distillation_weights[lang_pair] = 0
            else:
                task2.RS_distillation_weights[lang_pair] = cfg2.task.distillation_weight
...
def search_better_distillation_weights(cfg: DictConfig, trainer: Trainer, task: tasks.FairseqTask, epoch_itr,
    ...
    load_RS_distillation_weights(cfg, trainer)
    ...
    trial_records = {}
    for lang_pair in task.RS_distillation_weights.keys():
    ...
    # print("i want to know if the model parameters on each GPU are the same at this time, so i print:")
    ...
    for trial_step in search_space:
        ...
        # change alpha
        for lang_pair, distillation_weight in task.RS_distillation_weights.items():
            ori_alpha = task.RS_distillation_weights[lang_pair]
            ...
            task.RS_distillation_weights[lang_pair] = alpha
        ...
        for i, samples in enumerate(progress):
            ...
            if end_of_trial_epoch:
                ...
                if trainer.is_data_parallel_master:
                    logger.debug(f"RS_valid_loss after trial: {RS_valid_loss}")

                # record the trial results.
                for lang_pair, val_loss in RS_valid_loss.items():
        ...
        # roll back
        for lang_pair, distillation_weight in task.RS_distillation_weights.items():
            alpha = task.RS_distillation_weights[lang_pair]
    ...
    trial_records looks like:
        { 'src1-tgt1': [6.340326520140255, 6.535490635422902, 6.87558864277773], 
          'src1-tgt7': [6.397146437727406, 6.578106132553365, 6.900015743245029]
        }
    """
...
def save_RS_distillation_weights(cfg, trainer):
    """
    This function writes the language-specific distillation-weights of GPU-0 into the save_dir/RS_distillation_weights.json

    """
    if trainer.is_data_parallel_master:
        data = ""
        if os.path.exists(f"{cfg.checkpoint.save_dir}/RS_distillation_weights.json"):
            f = open(f"{cfg.checkpoint.save_dir}/RS_distillation_weights.json")
            data = f.read()
            f.close()

        last_RS_distillation_weights = json.dumps(trainer.task.RS_distillation_weights)
        data += f"{last_RS_distillation_weights}\n"

        f = open(f"{cfg.checkpoint.save_dir}/RS_distillation_weights.json", 'w+')
        f.write(data)
        f.close()


def load_RS_distillation_weights(cfg, trainer):
    ...
    if os.path.exists(f"{cfg.checkpoint.save_dir}/RS_distillation_weights.json"):
        ...
        last_RS_distillation_weights = data.strip().split('\n')[-1]
        last_RS_distillation_weights = json.loads(last_RS_distillation_weights)

        for lang_pair, weight in last_RS_distillation_weights.items():
            trainer.task.RS_distillation_weights[lang_pair] = weight
