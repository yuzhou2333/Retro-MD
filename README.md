# Retrosynthesis Mutual-Distillation

A deep learning framework for retrosynthesis prediction using mutual distillation techniques.
...
```bash
unzip uspto50k_aug20.zip -d data_bin/
```
...
```
bash ./PCL_scripts/uspto50k_aug20/Retro-MD.sh
```
...
```
bash ./PCL_scripts/uspto50k_aug20/test2.sh
```
...
Move the following files from the main directory to your prediction results folder (e.g., `./results/Retro-MD/157`):
...
### 9. Evaluate Different Test Sets
...
🔗 [Google Drive](https://drive.google.com/drive/folders/1mZxU2AE51wKraAX3SFA1PewSdvvtPrUY)
...
DATA_BIN=/path/to/data_bin/uspto50k_aug20 \
bash inference.sh
```
...
Retro-MD uses reaction-specific source and target names by default: `src{target}` to `tgt{target}`. Override `SOURCE_TEMPLATE` or `LANG_ARG_STYLE` only if your checkpoint was trained with a different language-pair convention.
