...
for file in result_files:
    ...
    reaction = re.match(result_file_mode, file).group(1)
...
for reaction_pair in reaction_pairs:
    print("%15s" % reaction_pair, end='')

print()

print(("%10s"*len(reaction_pairs)) % tuple([bleus[reaction] for reaction in reaction_pairs]))

print()

print("avg:", sum([float(bleus[reaction]) for reaction in reaction_pairs]) / len(bleus))