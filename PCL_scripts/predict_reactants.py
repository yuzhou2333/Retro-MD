#!/usr/bin/env python3
"""Run Retro-MD retrosynthesis inference for product SMILES.

The trained Retro-MD model is a multilingual fairseq translation model:
product SMILES are mapped through reaction-specific source/target names
(src1..src10 to tgt1..tgt10). This script tokenizes raw SMILES into the
...
DEFAULT_DATA_BIN = PROJECT_ROOT / "data_bin" / "uspto50k_aug20"
DEFAULT_REACTION_DICT = DEFAULT_DATA_BIN / "reaction_list.txt"
DEFAULT_TARGETS = tuple(range(1, 11))
DEFAULT_SOURCE_TEMPLATE = "src{target}"
DEFAULT_LANG_ARG_STYLE = "reaction"
...
def resolve_checkpoint(path: Optional[str]) -> Path:
    ...
    candidates = [
        PROJECT_ROOT / "save_models" / "Retro_MD" / "checkpoint_best.pt",
        PROJECT_ROOT / "save_models" / "Retro_MD" / "checkpoint_last.pt",
        PROJECT_ROOT / "save_models" / "Retro_MD" / "model2" / "checkpoint_best.pt",
        PROJECT_ROOT / "checkpoints" / "uspto50k_aug20" / "Retro_MD" / "model2" / "checkpoint_best.pt",
    ]
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Predict retrosynthesis reactants for product SMILES with a trained Retro-MD model."
    )
